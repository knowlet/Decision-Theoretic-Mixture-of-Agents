...
After refitting, Bellman, static and myopic obtain the same objective on every paired binary test case, with macro $J=@@REFIT_OBJECTIVE@@$. This is stronger than merely failing to reject a difference in this realized sample, but it does not establish population equivalence. Their batch schedules can differ even when selected candidates and acquisition counts match. On GSM8K and SVAMP, one query suffices for these selected policies; MMLU and StrategyQA use two.
