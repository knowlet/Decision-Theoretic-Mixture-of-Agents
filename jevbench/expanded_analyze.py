"""Fail-closed analysis for the v1.8 expanded native-head cohort."""
...
def run(incoming: Path, fixture: Path, evaluation: Path, out: Path) -> None:
    ...
    expected_cases = int(fixture_manifest["cases"])
    if len(qmap) != expected_cases or set(qmap) != set(gmap):
    ...
    for model in EXPANDED_MODEL_ORDER:
        ...
        for root in roots:
            ...
            if prov["model"] != model or prov["adapter"]["id"] != EXPANDED_MODELS[model]["repo"]:
                raise ValueError("model provenance mismatch")
            if prov["adapter"]["revision"] != EXPANDED_MODELS[model]["revision"]:
                raise ValueError("checkpoint revision mismatch")
            if prov["fixture_sha256"] != fixture_manifest["requests_sha256"]:
                raise ValueError("inference fixture provenance mismatch")
            if prov["protocol_sha256"] != fixture_manifest.get("protocol_sha256"):
                raise ValueError("inference protocol provenance mismatch")
            if prov["warmups"] != 1 or prov["new_api_calls"] != 0 or prov["new_training_steps"] != 0:
                raise ValueError("unexpected warmup, external call, or training activity")
        ...
        if sum(p["primary"] for p in provenance if p["model"] == model) != expected_cases:
            raise ValueError(f"primary provenance count mismatch for {model}")
        expected_probe_records = 4 * len(DATASETS) + 2 * len(DATASETS)
        if sum(p["forwards"] for p in provenance if p["model"] == model) != expected_cases + expected_probe_records:
    ...
    frame["leaderboard"] = frame["dataset"].map({dataset: board for board, datasets in LEADERBOARDS.items() for dataset in datasets})
    ...
    if len(frame) != expected_cases * len(EXPANDED_MODEL_ORDER) or len(test) != SIZES["test"] * len(DATASETS) * len(EXPANDED_MODEL_ORDER):
        raise ValueError("primary/test row counts are incomplete")
    if probe_counts != {"reverse": 4 * len(DATASETS) * len(EXPANDED_MODEL_ORDER), "repeat": 2 * len(DATASETS) * len(EXPANDED_MODEL_ORDER)}:
        raise ValueError(f"probe coverage mismatch: {probe_counts}")
    if any(len(group) != SIZES["test"] for _, group in test.groupby(["model", "dataset"])):
        raise ValueError("test cohort is not 200 rows per model and dataset")
    leaderboard_rows = []
    for (model, leaderboard), group in test.groupby(["model", "leaderboard"], sort=True):
        leaderboard_rows.append({
            "model": model,
            "leaderboard": leaderboard,
            "datasets": ",".join(LEADERBOARDS[leaderboard]),
            "n": len(group),
            "correct": int(group["correct"].sum()),
            "accuracy": float(group["correct"].mean()),
            "brier": float(group["brier"].mean()),
    ...
    frame.to_csv(out / "primary_predictions.csv", index=False)
    pd.DataFrame(summary).to_csv(out / "native_summary.csv", index=False)
    pd.DataFrame(leaderboard_rows).to_csv(out / "leaderboard_summary.csv", index=False)
    write(out / "verification.json", {
        ...
        "primary_forwards": expected_cases * len(EXPANDED_MODEL_ORDER),
        "evidence_forwards": (expected_cases + expected_probe_records) * len(EXPANDED_MODEL_ORDER),
        "probe_forwards": expected_probe_records * len(EXPANDED_MODEL_ORDER),
        "warmup_forwards": 12 * len(EXPANDED_MODEL_ORDER),
        "total_forwards": (expected_cases + expected_probe_records + 12) * len(EXPANDED_MODEL_ORDER),
        ...
        "scope": "v1.8 expanded native typed-choice benchmark; English and OOD-Chinese leaderboards; separate from v1.7 pilot",
    })
