"""Adapters for the v1.8 expanded native decision benchmark.
...
from .expanded_registry import EXPANDED_MODELS
...
class ExpandedAdapter:
    ...
    def __init__(self, name: str, vendor: Path = Path("vendor")):
        if name not in EXPANDED_MODELS:
            raise ValueError(f"unknown extended model: {name}")
        self.name = name
        self.spec = EXPANDED_MODELS[name]
        ...
        self.meta = {
            ...
            "leaderboard_role": self.spec.get("leaderboard_role"),
            "dtype": self.dtype,
            "device": "cpu",
            "torch_threads": 4,
    ...
    def _load_laya(self) -> None:
        ...
        self.dtype = "float32"
        if self.spec["repo"].endswith("laya-multilingual"):
            self.layout = "upstream mmBERT multilingual encoder with typed choice head; fp32 evaluation"
        else:
