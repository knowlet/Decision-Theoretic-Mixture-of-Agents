...
In GitHub Actions, **OpenJev versus decision-theoretic controls** executes actual local selector inference on two immutable Qwen3.5 checkpoints. Fifteen CPU workers execute the locked 192 test / 96 development cases across six primary datasets, with disjoint shards and same-worker reversal/repeat probes. They do not call proprietary TypeSafe Jev or generate new worker answers. Requests are built from a canonical fixture: candidate error estimates are rounded to eight decimal places before rendering, so the same case serializes identically on every shard and in the analysis job. The first v1.5 attempt failed without this, because the shards and the analysis job install different pinned numpy builds; the rounding removes that failure mode by construction.

Read `openjev_protocol.json` and `OPENJEV_RUNTIME_CHANGELOG.md` together: the latter discloses FP32 Linear/Conv accumulation under BF16 weight/output storage and per-worker warmups.
